...
namespace @delegate
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IEnumerable<int> ltTenResult = RunListThroughGauntlet(hugeList, ltTen);
            Console.WriteLine("All values in hugeList less than 10:");
            foreach(int n in ltTenResult)
            {
                Console.WriteLine(n);
            }
            IEnumerable<int> ltFiftyResult = RunListThroughGauntlet(hugeList, ltFifty);
            Console.WriteLine("-----------------------------------\nAll values in hugeList less than 50:");
            foreach(int n in ltFiftyResult)
            {
                Console.WriteLine(n);
            }
            IEnumerable<int> gtFiveHunResult = RunListThroughGauntlet(hugeList, gtFiveHun);
